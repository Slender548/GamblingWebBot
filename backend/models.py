...
class FinishedGame(Model):
    ...
    first_user_id: Mapped[int] = mapped_column(ForeignKey('users.user_id'))
    second_user_id: Mapped[int | None] = mapped_column(
        ForeignKey('users.user_id'), nullable=True)
    amount: Mapped[int] = mapped_column(nullable=False)
    game_hash: Mapped[int] = mapped_column(default=str(uuid4()))
...
class Referrals(Model):
    ...
    referrer_id: Mapped[int] = mapped_column(ForeignKey('users.user_id'),
                                             nullable=False)
    referred_id: Mapped[int] = mapped_column(ForeignKey('users.user_id'),
                                             nullable=False)
...
class Transactions(Model):
    ...
    transaction_data: Mapped[str] = mapped_column(nullable=False)
...
#     print(asyncio.run(get_user(2134)).username)
#     # asyncio.run(
#     #     insert_to_table(telegram_id="642",
#     #                     username="howdidyougetthere",
#     #                     wallet_address="maybe"))
#     asyncio.run(select_table(2))
