...
document.addEventListener('DOMContentLoaded', async () => {
    try {
        const data = await fetchBlackjackRooms();
        if (data) {
            ...
            rooms.forEach(room => {
                ...
                button.addEventListener('click', () => {
                    ...
                    fetch(`/try_join_blackjack_game?room_id=${room.room_id}`)
                    .then(response => {
                    ...
                    .then(data => {
                        if (data.error) {
                            ...
                            window.location.href = `/join_blackjack_game?room_id=${room.room_id}&player_id=p2&reward=${room.reward}`;
                        }
                    })
                });
...
async function fetchBlackjackRooms() {
    try {
        const response = await fetch('/get_blackjack_rooms');
        if (!response.ok) {
...
async function createBlackjackGame(name, reward) {
const response = await fetch(`/create_blackjack_game?name=${name}&reward=${reward}`);
if (response.ok) {
    return await response.json();
} else {
...
document.getElementById('createForm').addEventListener('submit', async function(event) {
    ...
    try {
        const data = await createBlackjackGame(name, reward);
        window.location.href = `/join_blackjack_game?room_id=${data.hash}&player_id=p1&reward=${data.reward}&creator=y`;
    } catch (error) {
        showAlertPopup("Произошла ошибка при создании игры");
    }
});
